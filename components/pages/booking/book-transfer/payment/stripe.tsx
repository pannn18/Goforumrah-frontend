...
const CheckoutForm = ({ idCarBooking, price, currentURL, onError, onSuccess }: Props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const result = await stripe.confirmPayment({
      ...
      elements,
      confirmParams: {
        return_url: origin + '/booking/book-transfer/stripe/callback',
        payment_method_data: {
          billing_details: {
    ...
    if (result.error) {
      ...
      const { ok } = await callAPI('/car-business-booking/add-payment', 'POST', { id_car_booking: idCarBooking, payment_intent: result.paymentIntent.id }, true)
      ...
      if (ok) {
        if (result.paymentIntent.status === 'succeeded') {
          ...
          const { ok } = await callAPI('/car-business-booking/cancel', 'POST', { id_car_booking: idCarBooking }, true)
