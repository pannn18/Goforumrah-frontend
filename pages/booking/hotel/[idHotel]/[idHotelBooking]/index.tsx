...
const Booking = (props: IProps) => {
  ...
  if (session.status === "unauthenticated") {
    router.push(`/hotel/detail?id=${data?.id_hotel}&checkin=${data?.checkin}&checkout=${data?.checkout}&status=unauthenticated`);
  }
  ...
  // console.log("data fetched on booking payment hotel : ", data)
    ...
    <>
      <Layout>
        ...
        <main className="booking-hotel booking-flight">
          ...
          <BookingPayment data={data} resolvedUrl={props.resolvedUrl} />
        </main>
        <Footer />
      </Layout>
    </>
