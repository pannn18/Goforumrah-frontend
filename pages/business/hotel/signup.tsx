...
const Signup = () => {
  ...
  const onSubmit = async (values) => {
    ...
    if (activeStep === 'password' && ((!isValid && !errors.password && !values.password && !errors.confirmPassword && !values.confirmPassword) || (isValid))) {
      ...
      const { ok, error } = await callAPI(`/hotel-business/store`, 'POST', payload)
              ...
              <div className='row h-100'>
                <div className='col-lg-6 col-lg-offset-6 col-md-12 signup-business__left'>
                  <div className='signup-business__left-content'>
                        ...
                        <div className="d-flex flex-column align-items-stretch text-center" style={{ marginTop: 24 }}>
                          <p className='text-danger-main'>
                            {error && error === 'Email or Username is already in use.'
                              ?
                              <>Email or Username is already in use, If you already have an account, <span className='d-block'>please proceed to the <Link href="/business/hotel/login" className='booking-hotel__aggreement-link'>login page</Link>.</span>
                              </>
                              : error}
                          </p>
                        </div>
                    ...
                    {activeStep === 'email' && (
                      <Link href='/business/hotel/login' className='btn goform-button goform-button--outline-green signup-business__form-button'>Login</Link>
                    )}
