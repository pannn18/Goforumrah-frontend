...
export default function InsuranceDocumentUpload() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const { status, data, ok, error } = await callAPI('/car-business/policies/store-insurance', 'POST', payloadFile, true, '', 'multipart/form-data', { onUploadProgress: (e) => setProggresBar(Math.floor((e.loaded / e.total) * 100)) })
